...
@if ($crud->route === 'admin/consolidated')
    ...
    <a href="{{ route('update_balance', ['year' => request('year')]) }}" class="btn btn-xs btn-danger"><i
            class="las la-clone"></i> Сформировать </a>
@endif
...
@if ($crud->route === 'admin/consolidateoboroti')
    <a href="{{ backpack_url('oborot-import') }}" class="btn btn-xs btn-success"><i class="la la-file-excel"></i> Импорт
        Обороты</a>
    <a href="{{ route('update_oboroti', ['year' => request('year')]) }}" class="btn btn-xs btn-danger"><i class="las la-clone"></i> Сформировать </a>
@endif
